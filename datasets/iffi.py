...
class IFFIDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            filtered_imgs = [Image.open(os.path.join(img_folder, filtered_img_fname)).convert("RGB")
                             for filtered_img_fname in filtered_img_fnames
                             # if filtered_img_fname.split(".")[0].split("_")[-1] not in ["Willow"]
                             ]
